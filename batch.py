...
def estimate_time(input_videos):
    ...
    for input_video in input_videos:
        ...
        output_path = input_video.replace(INPUT_VIDEOS_FOLDER,
                                          OUTPUT_VIDEOS_FOLDER).replace(
                                              input_video_fullname,
                                              output_video_fullname)
        if not os.path.exists(output_path):
            input_videos_unprocessed.append(input_video)

    total_time_second = 0
    for input_video in input_videos_unprocessed:
        # print(input_video, end='\t')
...
if __name__ == '__main__':
    ...
    for index, input_video in enumerate(input_videos):
        ...
        try:
            ...
            nb_frames = int(video_info_dict['nb_frames'])
            try:
                bit_rate = int(int(video_info_dict['bit_rate']) * 1.1)
            except KeyError:
                bit_rate = 0
        except Exception:
            timed_log(f'视频解析出错：{input_video}\n\n\n')
            with open(os.path.join(LOG_FOLDER, 'error.log'),
                      'a',
                      encoding='utf8') as f:
                f.write(f'视频解析出错：{input_video}')
        ...
        # 判断是否存在音频流
        audio_exists = len(
            [c for c in info['streams'] if c['codec_type'] == 'audio']) > 0
        ...
        timed_log(f'开始解帧，指令: {extract_frames_cmd}')
        p = subprocess.Popen(
            extract_frames_cmd,
            stderr=subprocess.STDOUT,
            stdout=open(f'{LOG_FOLDER}/{output_video_fullname}_extract.log',
                        'a'))
        p.wait()
        time_extract_end = time()
        time_consumption = time_extract_end - time_extract_start
        timed_log(
            f'解帧完成, 用时 {round(time_consumption, 2)} 秒, 速度 {round(nb_frames / time_consumption, 2)} fps'
        )
        ...
        timed_log(f'开始超分，指令: {upsampling_cmd}')
        p = subprocess.Popen(upsampling_cmd,
                             stderr=subprocess.STDOUT,
        ...
        timed_log(
            f'超分完成, 用时 {round(time_consumption, 2)} 秒, 速度 {round(nb_frames / time_consumption, 2)} fps'
        )
        ...
        # 如果原有码率检测不到，则仍然使用 CPU codec hevc，自动推断码率
        codec_command = f'-c:v hevc_nvenc -b:v {bit_rate}' if bit_rate != 0 else '-c:v hevc'
        rebuild_video_cmd = f'ffmpeg -r {frame_rate} -f image2 -i {OUTPUT_IMAGE_FOLDER}/frame%08d.jpg -i \"{input_video}\" -map 0:v:0 {"-map 1:a:0" if audio_exists else ""} -c:a copy {codec_command} -r {frame_rate} -pix_fmt yuv420p \"{output_path}\" -y'
        timed_log(f'开始合成，指令: {rebuild_video_cmd}')
        p = subprocess.Popen(
            rebuild_video_cmd,
            stderr=subprocess.STDOUT,
            stdout=open(f'{LOG_FOLDER}/{output_video_fullname}_rebuild.log',
                        'a'))
        p.wait()
        time_rebuild_end = time()
        time_consumption = time_rebuild_end - time_rebuild_start
        timed_log(
            f'合成完成, 用时 {round(time_consumption, 2)} 秒, 速度 {round(nb_frames / time_consumption, 2)} fps'
        )

        timed_log(f'输出视频: {output_path}')
        processed_videos_num += 1

        tv1 = int(time())
        timed_log(f'该视频耗时 {tv1 - tv0} 秒')
        ...
        timed_log(f'累计耗时 {total_h} 小时 {total_m} 分 {total_s} 秒')
        avg_processing_time = total_processing_time / (processed_videos_num)
        timed_log(f'平均单个视频耗时 {avg_processing_time} 秒')
        ...
        timed_log(
            f'预估剩余 {predicted_h} 小时 {predicted_m} 分 {predicted_s} 秒\n\n\n')
    refresh_frames_folder()
"""
index : 0
codec_name : h264
